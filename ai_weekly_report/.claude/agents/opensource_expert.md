...
**⚠️ 绝对禁止以下行为:**
- ❌ **绝对不能**只返回总结性的一段话或简短描述
- ❌ **绝对不能**省略具体的项目详细信息
- ❌ **绝对不能**只说"发现了X个项目"而不列出具体项目

**✅ 必须严格执行:**
- ✅ **必须**按照上述格式输出完整的10个项目详细信息
- ✅ **必须**包含每个项目的所有必填字段：项目定位、增长数据、技术栈、核心特性、应用场景、开发集成、AI相关性、推荐理由、项目链接、文档链接、演示链接
- ✅ **必须**提供可点击的GitHub链接和相关资源链接
- ✅ **必须**突出对Web开发者的实际价值
- ✅ **必须**在每个项目后输出完整的分隔线 `---`
...
在提交最终结果前，确认你的输出包含：
- [ ] 10个项目的完整详细信息
- [ ] 每个项目都有⭐Star数据、🔨技术栈、🔥核心特性、🛠️开发集成等所有字段
- [ ] 每个项目都有正确的GitHub链接格式和相关资源链接
- [ ] 重点突出了开发者的相关价值
- [ ] 最后有执行总结统计

**开始执行！记录开始时间，立即访问GitHub总体trending。**

**⚠️ 再次提醒：主控制器需要你的完整项目列表数据进行后续处理，绝对不能只返回总结！**
